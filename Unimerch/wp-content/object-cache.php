...
/**
 * We are using v2.0.0+
 * To be sure that everything will work fine, we're going to check the EasyWP Plugin.
 */
function easywp_plugin_exists()
{
    $plugin_dir = defined('WPMU_PLUGIN_DIR') ? WPMU_PLUGIN_DIR : WP_CONTENT_DIR . '/mu-plugins';

    return file_exists($plugin_dir . '/wp-nc-easywp/plugin/Http/Redis/includes/predis.php');
}

// phpcs:disable Generic.WhiteSpace.ScopeIndent.IncorrectExact, Generic.WhiteSpace.ScopeIndent.Incorrect
if ( (! defined( 'WP_REDIS_DISABLED' ) || ! WP_REDIS_DISABLED) && easywp_plugin_exists() ) :
